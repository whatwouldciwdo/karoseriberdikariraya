...
import { deletePortfolioProject } from "@/app/admin/_actions/portfolio";
...
export default function AdminPortfolioPage() {
  ...
  const fetchRows = async () => {
    ...
    const { data } = await supabase
      .from("portfolio")
      .select("slug, title, category, client, year")
      .order("sort_order");
    setRows(data ?? []);
    setLoading(false);
  };
    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-zinc-500 text-sm mt-1">{rows.length} proyek</p>
        </div>
        <Link href="/admin/portfolio/new" className="flex items-center gap-2 bg-red-500 hover:bg-red-600 text-white px-4 py-2.5 rounded-xl text-sm font-medium transition-colors">
      ...
      {loading ? (
        ...
        <div className="text-zinc-500 text-center py-16">Belum ada proyek.</div>
      ) : (
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-zinc-800">
                ...
                <th className="text-left px-6 py-4 text-xs font-medium text-zinc-500 uppercase tracking-wider">Klien</th>
                <th className="text-left px-6 py-4 text-xs font-medium text-zinc-500 uppercase tracking-wider">Tahun</th>
            ...
            <tbody>
              {rows.map((row) => (
                <tr key={row.slug} className="border-b border-zinc-800/50 hover:bg-zinc-800/30 transition-colors">
                  ...
                  <td className="px-6 py-4 text-sm text-zinc-400">{row.client}</td>
                  <td className="px-6 py-4 text-sm text-zinc-500">{row.year}</td>
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <Link href={`/admin/portfolio/edit?slug=${row.slug}`} className="p-2 hover:bg-zinc-700 rounded-lg transition-colors text-zinc-400 hover:text-zinc-200">
                        <Pencil size={14} />
                      </Link>
